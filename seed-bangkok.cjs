#!/usr/bin/env node
/**
 * Seeds Firestore with seed-bangkok-final.json
 * Run from project root: node seed-bangkok.js
 * Requires: gcloud CLI authenticated as project owner
 *   gcloud auth login
 *   gcloud config set project plzgo-bf50c
 */

const { execSync } = require('child_process')
const https = require('https')
const places = require('../seed-bangkok-final.json')
...
async function seed() {
  ...
  try {
    ...
    console.error('Could not get gcloud token. Run: gcloud auth login')
    process.exit(1)
  }

  console.log(`\nSeeding ${places.length} Bangkok places -> Firestore (${PROJECT_ID})\n`)
  ...
  for (const place of places) {
    ...
    try {
      ...
      console.log(`  OK   ${place.name_en}`)
      ok++
    } catch (e) {
      console.error(`  FAIL ${place.name_en}  ->  ${e.message}`)
      fail++
    }
  }
