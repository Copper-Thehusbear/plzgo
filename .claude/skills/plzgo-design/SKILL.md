...
## Physical language (2026-08) — อ่านก่อนทำ UI ทุกครั้ง
สีกับฟอนต์คงเดิมทุกตัว สิ่งที่เปลี่ยนคือ**พื้นผิวประพฤติตัวยังไง** — primitive กลางอยู่ใน `src/style.css` หัวข้อ "PHYSICAL LANGUAGE" ใช้ตัวพวกนี้ ห้ามประดิษฐ์ treatment ใหม่ต่อ component:
`.plz-eyebrow` (label mono มีเส้นนำ) · `.plz-secnum` (เลขบทยักษ์จาง ต้อง map กับข้อมูลจริง) · `.plz-paper` + `.plz-paper-tilt-l/-r` (กระดาษเอียง เงาลึก hover แล้วตั้งตรง) · `.plz-stamp` (ตราปั๊มขอบหนา เอียง ล้ำขอบ parent ได้) · `.plz-taped` (เทป 2 ชิ้นคนละมุม) · `.plz-perf` / `.plz-notched` (รอยปรุ / รอยบากตั๋ว) · `.plz-dotgrid` / `.plz-glow` (พื้นผิว) · `.plz-trail` (หมุดต่อกันด้วยลูกศร)

**กติกา:** ของที่เป็น "กระดาษ" ต้องเอียง (ชิ้นเล็ก ~2°, ชิ้นกว้าง ~1° — ใช้องศาเท่ากันบนของกว้างจะดูเหมือนบั๊ก) · เงาต้องลึกและกว้าง (`--shadow-md/-lift`, บนพื้น ink ใช้ `--shadow-deep`) ให้รู้สึกว่าวางอยู่**บน**พื้นผิว · ตราปั๊ม/เทปต้องล้ำขอบออกมา ไม่ใช่อยู่ในกรอบเรียบร้อย
**สาเหตุที่ของเดิมโดนด่าว่าดู AI:** ทุกการ์ดคือสี่เหลี่ยมขาวมนขอบเส้นบางเหมือนกันหมด — ต้องแยก treatment ตาม "มันคืออะไรจริงๆ" (ใบเสร็จ/ตั๋ว/โน้ตแปะ/ป้ายสถานี)
